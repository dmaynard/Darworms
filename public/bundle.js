...
window.addEventListener("load", function() {
    ...
    $("#aboutpage").on('pageshow', darworms$1.main.sendAnalytics);
    $("#debugpage").on('pageshow', darworms$1.main.sendAnalytics);
    $("#emailpage").on('pageshow', darworms$1.main.sendAnalytics);
...
function addSprite(point, dir, phase, colorIndex, ts, te) {
  const sprited = {
    dir: dir,
    phase: phase,
...
function drawSprite(cell, progress, dir, phase, colorIndex) {
  ...
  if (phase === 0) {
    ...
    wGraphics.lineTo(xPts$1[dir] * progress / 2.0, yPts$1[dir] * progress / 2.0);
    wGraphics.stroke();
    wGraphics.closePath();
  }
  if (phase === 1) {
    ...
    wGraphics.beginPath();
    wGraphics.moveTo(xPts$1[dir] * (progress) / 2.0, yPts$1[dir] * (progress) / 2.0);
    wGraphics.lineTo(xPts$1[dir] / 2.0, yPts$1[dir] / 2.0);
    wGraphics.stroke();
    wGraphics.closePath();
  }
  if (phase === 2) {
    ...
    wGraphics.beginPath();
    wGraphics.moveTo(xPts$1[dir] / 2.0, yPts$1[dir] / 2.0);
    wGraphics.lineTo(xPts$1[dir] * (1.0 - progress) / 2.0, yPts$1[dir] * (1.0 - progress) / 2.0);
    wGraphics.stroke();
    wGraphics.closePath();
  }
  if (phase === 3) {
    ...
    wGraphics.beginPath();
    wGraphics.moveTo(xPts$1[dir] * (1.0 - progress) / 2.0, yPts$1[dir] * (1.0 - progress) / 2.0);
  ...
  makeMove(graphicsOn) {
    ...
    for (var i = nextToMove; i < this.worms.length; i = i + 1) {
      ...
      if (currentState == 0x3F) {
        ...
        {
          addSprite ( active.pos, direction, 0, active.colorIndex, darworms$1.graphics.now , darworms$1.graphics.now + (darworms$1.graphics.frameInterval/4));
          addSprite ( active.pos, direction, 1, active.colorIndex, darworms$1.graphics.now + (darworms$1.graphics.frameInterval/4), darworms$1.graphics.now + (darworms$1.graphics.frameInterval/2));
      }
        ...
        if (next.isEqualTo(darworms$1.dwsettings.noWhere)) { // fell of edge of world
          ...
          {
            addSprite ( next, darworms$1.inDir[direction], 2, active.colorIndex, darworms$1.graphics.now + (darworms$1.graphics.frameInterval/2) , darworms$1.graphics.now + (darworms$1.graphics.frameInterval*3/4));
            addSprite ( next, darworms$1.inDir[direction], 3, active.colorIndex, darworms$1.graphics.now + (darworms$1.graphics.frameInterval*3/4), darworms$1.graphics.now + (darworms$1.graphics.frameInterval));
...
darworms$1.main = (function() {
  ...
  var applySettings = function() {
    ...
    if (newGridSize !== darworms$1.dwsettings.gridSize) {
      darworms$1.dwsettings.gridSize = newGridSize;
      try {
        ga('send', 'event', 'gridSize', newGridSize);
      } catch (err) {}
    }
    var newFps = parseInt($("#fps").val());
    if (newFps !== darworms$1.graphics.fps) {
      darworms$1.graphics.fps = newFps;
      try {
    ...
    darworms$1.dwsettings.doAudio = $('#audioon').slider().val() == "1" ? 1 : 0;    darworms$1.dwsettings.fixedInitPos = $('#fixedinitpos').slider().val() == "1" ? 1 : 0;    darworms$1.dwsettings.pickDirectionUI = $('#pickDirectionUI').slider().val() == "1" ? 1 : 0;
  ...
  darworms$1.setInstrumentVal = function(index) {
    ...
    if (selectedInstrumentInput.length == 1) {
      ...
      try {
        ga('send', 'event', 'instrument', darworms$1.audiosamplefiles[instrument][0]);
      } catch (err) {}
    ...
    if (darworms$1.theGame.gameState === darworms$1.gameStates.running) {
      ...
      if (darworms$1.dwsettings.doAnimations) {
        ...
        if (darworms$1.graphics.elapsed > darworms$1.graphics.frameInterval) {
          ...
          animateSprites(darworms$1.graphics.now);
        }
      } else {
  ...
  var init = function() {
    ...
    var when;
    try {
      when  = new Date(darworms$1.builddate);
    } catch (err) {
      when = Date.now();
    }
    ...
    $("input[name='edit-radio-choice']").on("change", function() {
      ...
      try {
        ga('send', 'event', 'wType', type, darworms$1.selectedIdx);
      } catch (err) {}
    });
